...
namespace BluetoothLEExplorer.ViewModels
{
    ...
    public class CharacteristicPageViewModel : ViewModelBase
    {
        ...
        {
            string path = @"C:\Users\IsraelB\Documents\IOS\json example\measurement.json"; //@ pour denoter une addresse
            IBuffer jsonBuffer = null;
        ...
        //New function for the split process
        public void SplitProcess(string str512, IBuffer interBuffer)
        {
            int chunkSize = 512;
            ...
            for (int i = 0; i < stringLength; i += chunkSize)
            {
                ...
                //These two are the equivalent of "debugOutput"
                interBuffer = CryptographicBuffer.ConvertStringToBinary(str512.Substring(i, chunkSize),
                BinaryStringEncoding.Utf8);
                WriteValueBuffer(interBuffer); //And finally here
            }
        }

        //The same old WriteValueBuffer void but with the parameter edit (readBuffer)
        public async void WriteValueBuffer(IBuffer readBuffer) //I moved it as a function to use the split structure
        {
            try
            {
                // BT_Code: Writes the value from the buffer to the characteristic.
                GattCommunicationStatus result = await Characteristic.Characteristic.WriteValueAsync(readBuffer);
